...
const { createOpenCodePushApproval } = require('./opencode-push-approval');
const { pushOpenCodeCommit } = require('./opencode-push');
...
function openCodePushApprovalResponseText(result) {
  if (!result.ok) return `OpenCode push approval blocked: ${result.reason}${jsonBlock(result)}`;
  return `OpenCode push approval requested. Push remains disabled until approval.${jsonBlock(result)}`;
}

function openCodePushResponseText(result) {
  if (!result.ok) return `OpenCode push failed: ${result.reason}${jsonBlock(result)}`;
  return `OpenCode push completed. Deploy remains disabled.${jsonBlock(result)}`;
}

function openCodePrApprovalResponseText(result) {
  if (!result.ok) return `OpenCode PR approval blocked: ${result.reason}${jsonBlock(result)}`;
  return `OpenCode PR approval requested. PR creation remains disabled until approval.${jsonBlock(result)}`;
}

function openCodePrResponseText(result) {
  if (!result.ok) return `OpenCode PR creation failed: ${result.reason}${jsonBlock(result)}`;
  return `OpenCode PR created. Merge and deploy remain disabled.${jsonBlock(result)}`;
}

function openCodeStatusResponseText(result) {
  if (!result.ok) return `OpenCode job status failed: ${result.reason}${jsonBlock(result)}`;
  return `OpenCode job status.${jsonBlock(result)}`;
}

function openCodeAbortResponseText(result) {
  if (!result.ok) return `OpenCode abort failed: ${result.reason}${jsonBlock(result)}`;
...
function handleTelegramCommand(parsed, context = {}) {
  ...
  if (parsed.type === 'opencode_push_approval') {
    const [commitSha, branch, remote] = parsed.args;
    const result = createOpenCodePushApproval({ rootDir, commit_sha: commitSha, branch, remote, allowed_user_ids: userId ? [userId] : [], now: context.now || new Date() });
    return textResponse(openCodePushApprovalResponseText(result), { result, summary: result, wired_to_runtime: false, execution_connected: false, push_allowed: false });
  }
  if (parsed.type === 'opencode_push') {
    const [approvalId] = parsed.args;
    const result = pushOpenCodeCommit({ rootDir, approval_id: approvalId, timeout_ms: context.timeout_ms, now: context.nowFn });
    return textResponse(openCodePushResponseText(result), { result, summary: result, wired_to_runtime: false, execution_connected: result.execution_connected === true, push_allowed: result.push_allowed === true });
  }
  if (parsed.type === 'opencode_pr_approval') {
    const [commitSha, headBranch, baseBranch, ...titleParts] = parsed.args;
    const result = createOpenCodePrApproval({ rootDir, commit_sha: commitSha, head_branch: headBranch, base_branch: baseBranch, title: titleParts.join(' '), allowed_user_ids: userId ? [userId] : [], now: context.now || new Date() });
    return textResponse(openCodePrApprovalResponseText(result), { result, summary: result, wired_to_runtime: false, execution_connected: false, pr_allowed: false });
  }
  if (parsed.type === 'opencode_pr') {
    const [approvalId] = parsed.args;
    const result = createOpenCodePullRequest({ rootDir, approval_id: approvalId, githubClient: context.githubClient, repository_full_name: context.repository_full_name, now: context.nowFn });
    return textResponse(openCodePrResponseText(result), { result, summary: result, wired_to_runtime: false, execution_connected: result.execution_connected === true, pr_allowed: result.pr_allowed === true });
  }
  if (parsed.type === 'opencode_status') {
    const [jobId] = parsed.args;
    const result = opencodeJobStatus({ rootDir, job_id: jobId || null });
    return textResponse(openCodeStatusResponseText(result), { result, summary: result, wired_to_runtime: false, execution_connected: false });
  }
  if (parsed.type === 'opencode_abort') {
    const [jobId] = parsed.args;
