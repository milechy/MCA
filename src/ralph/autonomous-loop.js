...
// Phase A7: classify the story's task once at PLAN time so the NemoClaw brain
// can route on task type + complexity. Opt-in (NEMOCLAW_CLASSIFIER=on) because
// the classifier spawns a Python model load (~seconds); default off keeps the
// loop and tests free of that cost. Falls back to any existing signal.
function maybeClassifySignal(story, env = process.env) {
  if ((env.NEMOCLAW_CLASSIFIER || 'off') !== 'on') return story.classifier_signal || null;
  try {
    return classifyPrompt({ prompt: taskForStory(story), env }) || story.classifier_signal || null;
  } catch {
    return story.classifier_signal || null;
  }
}
...
// Phase A7: when a story first enters a terminal phase, record the outcome to
// .ralph/outcomes.jsonl so the NemoClaw brain has live learning data (the data
// end of the loop was previously open — recordTaskOutcome was never called).
// Best-effort: never break the loop on a recording failure.
function maybeRecordTerminalOutcome({ rootDir = process.cwd(), now = new Date(), env = process.env, result } = {}) {
  if (!result || !result.story_id) return;
  const TERMINAL = [LOOP_PHASES.DONE, LOOP_PHASES.ESCALATED];
  // Only on the tick where it FIRST becomes terminal (skip the entry
  // short-circuit ticks where from_phase is already terminal).
  if (!TERMINAL.includes(result.to_phase) || TERMINAL.includes(result.from_phase)) return;
  try {
    const story = readStory(rootDir, result.story_id);
    if (story) recordStoryOutcome({ rootDir, story, env, now });
