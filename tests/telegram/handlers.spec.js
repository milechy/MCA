...
function makeTempRoot(initialMode = {}) {
  ...
  fs.writeFileSync(path.join(rootDir, '.ralph', 'state.json'), `${JSON.stringify({ loop_id: 'telegram-test', phase: 'IDLE', current_story_id: null, iteration: 0, consecutive_failures: 0, active_mode: 'approval', current_approval_id: null, last_green_commit: null, security_stop: false, updated_at: '2026-05-06T00:00:00+09:00' }, null, 2)}\n`, 'utf8');
  fs.writeFileSync(path.join(rootDir, '.ralph', 'mode.json'), `${JSON.stringify({ mode: MODES.APPROVAL, effective_until: null, auto_revert_to: null, changed_by: { channel: 'manual', user: 'test' }, policy_version: 'approval-policy-v1.4', reason: 'telegram_test_setup', updated_at: '2026-05-06T00:00:00+09:00', ...initialMode }, null, 2)}\n`, 'utf8');
  return rootDir;
}
...
function sampleRunAllPlan() {
  return { story_id: 'STORY-TELEGRAM-RUN-ALL-PREFLIGHT', mode: 'approval', target_env: 'staging', summary: 'Telegram run-all preflight test', objective: 'Validate Telegram run-all preflight without executing shell', planned_files: [], migration_plan: { target: 'staging', sql: '' }, allowed_user_ids: [3] };
}
...
function createApprovedPlan(rootDir, plan) {
  const approval = createApproval(plan, evaluateRisk(plan), { rootDir, approval_id: 'APR-TELEGRAM-RUN-ALL', allowed_user_ids: [3], pre_exec_diff_hash: EMPTY_DIFF_HASH });
  return approveApprovalRecordOnly(approval.approval_id, 3, { rootDir, channel: 'telegram' });
}

function update(userId, chatId, text) {
  return { message: { text, from: { id: userId }, chat: { id: chatId } } };
}
...
test('/status returns state and mode', () => {
  ...
  const result = handleTelegramCommand(parseTelegramCommand('/status'), { rootDir, user_id: 3, roles: roles() });
  expect(result.ok).toBe(true);
...
test('/policy returns read-only execution policy status with runtime gate off', () => {
  ...
  expect(result.policy.telegram_run_all_enabled).toBe(false);
  expect(result.policy.telegram_run_all_default_off).toBe(true);
...
test('/policy returns read-only execution policy status with runtime gate true', () => {
  const result = handleTelegramCommand(parseTelegramCommand('/policy'), { rootDir: makeTempRoot(), user_id: 3, roles: roles(), env: { [TELEGRAM_RUN_ALL_ENV]: 'true' } });
...
test('/run-all performs preflight only and does not execute shell when env gate is off', () => {
  ...
  expect(result.summary).toMatchObject({ ok: true, reason: 'READY_BUT_NOT_EXECUTED', command: 'scripts/gates/run-all.sh', run_all_enabled: false, wired_to_runtime: false, execution_connected: false, commands_executed: [], files_modified: [], log_path: '.ralph/logs/execution.jsonl' });
...
test('/run-all executes allowlisted shell command when env gate is true', () => {
  ...
  const planPath = writeTmpPlan(rootDir, 'run-all-plan.json', plan);
  const approval = createApprovedPlan(rootDir, plan);
  const result = handleTelegramCommand(parseTelegramCommand(`/run-all ${approval.approval_id} ${planPath}`), { rootDir, user_id: 3, roles: roles(), env: { [TELEGRAM_RUN_ALL_ENV]: 'true' } });
  expect(result.ok).toBe(true);
  ...
  expect(result.result.files_modified).toEqual([]);
  expect(result.summary).toMatchObject({ ok: true, executor: 'shell', command: 'scripts/gates/run-all.sh', exit_code: 0, run_all_enabled: true, wired_to_runtime: true, execution_connected: true, commands_executed: ['scripts/gates/run-all.sh'], files_modified: [], log_path: '.ralph/logs/execution.jsonl' });
...
test('/run-all rejects unsafe or missing plan path before shell execution', () => {
  const result = handleTelegramCommand(parseTelegramCommand('/run-all APR-001 ../plan.json'), { rootDir: makeTempRoot(), user_id: 3, roles: roles() });
  expect(result.ok).toBe(true);
  expect(result.wired_to_runtime).toBe(false);
...
test('/mode fullauto creates token but does not immediately switch mode', () => {
  const rootDir = makeTempRoot();
  const result = handleTelegramCommand(parseTelegramCommand('/mode fullauto 6'), { rootDir, user_id: 2, roles: roles() });
  expect(result.ok).toBe(true);
  expect(result.result.ok).toBe(true);
...
test('/confirm enables fullauto after a valid fullauto request', () => {
  const rootDir = makeTempRoot();
  const request = handleTelegramCommand(parseTelegramCommand('/mode fullauto 6'), { rootDir, user_id: 2, roles: roles() });
  const confirm = handleTelegramCommand(parseTelegramCommand(`/confirm ${request.result.token}`), { rootDir, user_id: 2, roles: roles() });
...
test('/mode approval switches back to approval for reviewer', () => {
  ...
  const result = handleTelegramCommand(parseTelegramCommand('/mode approval'), { rootDir, user_id: 3, roles: roles() });
  expect(result.ok).toBe(true);
  expect(result.result.ok).toBe(true);
  expect(loadMode(rootDir).mode).toBe(MODES.APPROVAL);
});
...
test('/approve routes through adapter and remains execution-disconnected', () => {
  const result = handleTelegramCommand(parseTelegramCommand('/approve APR-001'), { rootDir: makeTempRoot(), user_id: 3, roles: roles() });
  expect(result.ok).toBe(true);
  expect(result.wired_to_runtime).toBe(false);
  expect(result.result.ok).toBe(false);
...
test('processTelegramUpdate rejects unauthorized user or chat', async () => {
  const rootDir = makeTempRoot();
  const badUser = await processTelegramUpdate(update(999, 10, '/ping'), { rootDir, config: allowedConfig(), roles: roles() });
  const badChat = await processTelegramUpdate(update(3, 99, '/ping'), { rootDir, config: allowedConfig(), roles: roles() });
...
test('processTelegramUpdate accepts authorized command', async () => {
  const result = await processTelegramUpdate(update(3, 10, '/ping'), { rootDir: makeTempRoot(), config: allowedConfig(), roles: roles() });
  expect(result.ok).toBe(true);
