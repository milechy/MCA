...
test('maybeFallbackAfterOpenCodeFailure converts eligible provider rate limit into PATCH_PREVIEW', () => {
  ...
  expect(story).toMatchObject({
    ...
    current_candidate_patch_path: '.ralph/tmp/opencode-sandbox/APR-FALLBACK/candidate.patch'
  });
  expect(fs.existsSync(path.join(rootDir, '.ralph/tmp/opencode-sandbox/APR-FALLBACK/candidate.patch'))).toBe(true);
...
test('ensurePushApprovalAfterCommit replaces commit completion with push approval metadata', () => {
  ...
  expect(result).toMatchObject({
    ...
    to_phase: LOOP_PHASES.PUSH_APPROVAL_PENDING,
    approval_id: 'APR-PUSH',
    push_approval: { requested_action: undefined, approval_id: 'APR-PUSH' },
